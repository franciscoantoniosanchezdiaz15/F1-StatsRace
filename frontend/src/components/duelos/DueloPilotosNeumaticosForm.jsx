...
export default function DueloPilotosNeumaticosForm({
  configDuelo,
  pilotoUsuario,
  onSubmit,
}) {
    ...
    <section className="max-w-5xl mx-auto px-10 py-10 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
        <div className="text-center md:text-left">
          <span className="text-[#FFEB00] font-mono text-xs uppercase tracking-[0.4em]">
            Pit Lane Strategy
          </span>
          <h1 className="text-5xl font-black italic text-white uppercase tracking-tighter mt-2">
            Elección de <span className="text-[#FFEB00]">Compuesto</span>
          </h1>
        </div>
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white font-bold italic">{pilotoUsuario.full_name}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        <div className="max-w-2xl mx-auto">
          <div className="bg-neutral-900 rounded-[32px] border border-neutral-800 overflow-hidden group hover:border-neutral-600 transition-all shadow-2xl">
            ...
            <div className="p-8 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                {COMPUESTOS.map((c) => {
                  const isSelected = compuestoUsuario === c.id;
                  return (
                    <button
                      key={c.id}
                      type="button"
                      onClick={() => setCompuestoUsuario(c.id)}
                      className={`
                    ...
                    >
                      <div
                        className={`w-10 h-10 rounded-full border-[6px] ${
                          isSelected ? c.border : "border-neutral-700"
                        } flex items-center justify-center bg-transparent shadow-inner`}
                      >
                        <div
                          className={`w-3 h-3 rounded-full ${
                            isSelected ? c.color : "bg-neutral-700"
                          }`}
                        ></div>
                      </div>
            ...
            className={`
              ...
              ${
                submitting
                  ...
                  : "bg-[#FFEB00] hover:scale-105 active:scale-95 shadow-[0_20px_50px_rgba(255,235,0,0.2)]"
              }
            `}
          >
