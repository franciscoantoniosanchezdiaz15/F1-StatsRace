...
export default function DueloPilotosCarreraPage() {
  ...
  async function handleContinue(payload, pilotos) {
    ...
    navigate("/duelos/pilotos/carrera/neumaticos", {
      state: {
        configDuelo: payload,
        pilotoUsuario,
      },
      ...
      <main className="max-w-6xl mx-auto px-6 mt-12">
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-neutral-500 max-w-2xl mx-auto text-sm font-medium uppercase tracking-wide">
            Establece las condiciones de pista y selecciona a tu rival para iniciar la simulación.
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <div className="bg-neutral-900/40 rounded-[48px] border border-white/5 p-4 md:p-8 backdrop-blur-sm">
            <DueloPilotosForm modo="carrera" onContinue={handleContinue} />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          <InfoCard text="Análisis de ritmo y consistencia basado en datos reales." />
          <InfoCard text="El circuito condiciona el rendimiento y la estrategia." />
          <InfoCard text="Comparativa directa 1v1 entre pilotos en igualdad de condiciones." />
        </div>
      </main>
    </div>
  );
