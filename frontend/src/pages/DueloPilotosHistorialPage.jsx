...
import {
  fetchHistorialDuelosPilotos,
  eliminarDueloPiloto,
} from "../services/dueloPilotosService";

export default function DueloPilotosHistorialPage() {
  ...
  async function cargarHistorial() {
    try {
      ...
      const data = await fetchHistorialDuelosPilotos();
      setHistorial(data);
    } catch (err) {
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-mono text-[#FFEB00] leading-none">
              {historial.length}
            </p>
          </div>
        </div>
        ...
        {!loading && !error && historial.length === 0 && (
          <div className="text-center py-20 bg-neutral-900/50 rounded-3xl border border-dashed border-neutral-700">
            <p className="text-gray-500 italic">
              No hay duelos de pilotos guardados en la base de datos.
            </p>
          </div>
        )}
...
function DueloCard({ duelo, onEliminar }) {
  ...
  const ganoUsuario = duelo.ganador === duelo.piloto1;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-6">
        <div className="flex-1">
          ...
          <h2 className="text-xl font-bold flex items-center gap-2">
            <span className={ganoUsuario ? "text-[#FFEB00]" : "text-white"}>
              {duelo.piloto1}
            </span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 lg:gap-8 border-t lg:border-t-0 lg:border-l border-neutral-800 pt-4 lg:pt-0 lg:pl-8">
          ...
          <div>
            <p className="text-[10px] uppercase text-gray-400 font-bold mb-1 italic">
              Rival
            </p>
          ...
          <div className="hidden md:block">
            <p className="text-[10px] uppercase text-[#FFEB00] font-black mb-1 italic tracking-widest">
              Distancia
            </p>
